...
export default function EditTournee({ id, nomActuel }: { id: string; nomActuel: string }) {
  ...
  async function enregistrer() {
    ...
    setBusy(true); setMsg('')
    const { error } = await supabase.from('tournees').update({ nom: nom.trim() }).eq('id', id)
    setBusy(false)
  ...
  return (
    <>
      <button
        ...
        aria-label="Renommer la tournée"
      >
        ✎
      </button>
