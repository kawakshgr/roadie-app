...
export default function EditDate({ d }: { d: DateData }) {
  ...
  // coordonnées si une nouvelle salle/ville est choisie dans l'autocomplétion
  ...
  async function enregistrer() {
    ...
    // si une nouvelle position a été choisie, on met à jour les coordonnées
    const maj: any = {
      ville: f.ville.trim(),
          ...
          <div className="modal glass" onClick={(e) => e.stopPropagation()}>
            ...
            <div className="modal-body">
              ...
              <p style={{ fontSize: 12, color: 'var(--ink-faint)', margin: '4px 4px 0' }}>
                Laisse vide pour garder le lieu actuel. Utilise la recherche pour le changer.
              </p>
              ...
              <div className="add-times">
                <div>
                  <div style={labelStyle}>Départ hôtel</div>
                  <input className="login-input" type="time" value={f.depart_hotel} onChange={(e) => set('depart_hotel', e.target.value)} />
                </div>
                <div>
                  <div style={labelStyle}>Km prochaine date</div>
                  <input className="login-input" type="number" placeholder="Ex : 320" value={f.km_prochaine} onChange={(e) => set('km_prochaine', e.target.value)} />
                </div>
              </div>
